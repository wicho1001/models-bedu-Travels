...
const Mutation = {
  ...
  createTour: async(_, { input }, { currentUser, userType, pubsub }) => {
    if (!currentUser) {
      throw new Error('Unauthorized');
    }

    // Allowed only for admins and agencies
    if (!['admin', 'agency'].includes(userType)) {
      throw new Error('Unauthorized');
    }
    ...
    pubsub.publish('tour-added', { newTour });
    return newTour;
  },
};
